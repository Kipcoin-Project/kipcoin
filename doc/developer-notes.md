...
- [Development guidelines](#development-guidelines)
    - [General Kipcoin Core](#general-kipcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Kipcoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/kipcoin-project/kipcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_kipcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_kipcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/kipcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_kipcoin.coverage/index.html`.
```

**Sanitizers**

Kipcoin Core can be compiled with various "sanitizers" enabled, which add
...
file descriptors it will cause problems with Kipcoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Kipcoin's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Kipcoin Core, the wallet was always
