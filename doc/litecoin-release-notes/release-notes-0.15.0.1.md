...
Kipcoin Core 0.15.0 contains the following changes to the RPC interface and `litecoin-cli` for multi-wallet:

* When running Kipcoin Core with a single wallet, there are **no** changes to the RPC interface or `litecoin-cli`. All RPC calls and `litecoin-cli` commands continue to work as before.
